...
public class MapAdmin implements Initializable {
    @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        if (loc2 != null) {
            ...
            loc2.setValue(SeeEventsAdmin.getsLoc());
        }
        if (host != null) {
            host.setText(SeeEventsAdmin.getsHost());
        }
        if (sDate != null) {
            sDate.setValue(SeeEventsAdmin.getseDate());
        }
        initList();
        initList2();
        ...
        if (SeeEventsAdmin.countId == 0) {
            ...
            try {
                ResultSet rs = HelloApplication.executeSelect("SELECT title FROM BrowseRSVPs" + SeeEventsAdmin.countId + " ORDER BY date_featured");
                //ResultSet rs2 = HelloApplication.executeSelect("SELECT ID FROM Events ORDER BY date_featured");
                ResultSet rs3 = HelloApplication.executeSelect("SELECT location FROM BrowseRSVPs" + SeeEventsAdmin.countId + " ORDER BY date_featured");
                String s = "";
                while(rs.next()  && rs3.next()) {
                    if (matchLocs.containsKey(rs3.getString(1))) {
                        s = matchLocs.get(rs3.getString(1));
    ...
    protected void handleOver(MouseEvent event) throws IOException {
        ...
        if (icons.get(id) != null) {
            SeeEventsAdmin.setCurrTitle(icons.get(id));
    ...
    protected void handleBack(MouseEvent event) throws IOException {
        SeeEventsAdmin.setsLoc(loc2.getValue());
        SeeEventsAdmin.setsHost(host.getText());
        SeeEventsAdmin.setseDate(sDate.getValue());
    ...
    @FXML void filterEvents(MouseEvent event) {
        try {
            ...
            count--;
            SeeEventsAdmin.setCountId(count);
    ...
    public void handleView(MouseEvent e) {
        try {
            ...
            ResultSet rs2 = HelloApplication.executeSelect("SELECT ID FROM Events WHERE title = '" + SeeEventsAdmin.currTitle + "';");
            if (rs2.next()) {
                SeeEventsAdmin.setsLoc(null);
                SeeEventsAdmin.setsHost("");
                SeeEventsAdmin.setseDate(null);
                SeeEventsAdmin.setCountId(0);
            ...
            ex.printStackTrace();
            System.out.println(SeeEventsAdmin.currTitle);
